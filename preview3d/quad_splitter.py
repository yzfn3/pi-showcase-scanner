...
def main():
    ...
    parser.add_argument("--make-mapping-sheet", action="store_true", help="Label the first step's four configured camera views")
    args = parser.parse_args()
    logging.basicConfig(level=logging.INFO, format="%(levelname)s %(message)s")
    try:
        if args.make_mapping_sheet and not needs_split(args.scan) and not args.dry_run:
            from windows_client.scan import load_manifest
            result = load_manifest(args.scan)
        else:
            result = split_scan(args.scan, args.config, dry_run=args.dry_run)
        if args.make_mapping_sheet and not args.dry_run:
            first = min(f['step'] for f in result['frames'])
            rows = [(args.scan/f['file'], f['step'], f['camera_id'], 'Verify camera identity and orientation')
                    for f in result['frames'] if f['step'] == first]
            contact_sheet(rows, args.scan/'outputs/inspection/camera_mapping_sheet.jpg')
